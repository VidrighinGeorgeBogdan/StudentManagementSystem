...
const router = Router();
import { get, create, update, remove } from '../services/classesService.js';
...
router.post('/create', async function(req, res, next) {
    try {
        res.json(await create(req.body.newClass, req.body.user));
    } catch (err) {
...
router.post('/update/:id', async function(req, res, next) {
    try {
        res.json(await update(req.params.id, req.body.newClass, req.body.user));
    } catch (err) {
